...
// GET /api/barang-keluar/cek-stok/:barang_id - Cek stok sebelum keluar
export const cekStokSebelumKeluar = async (req, res) => {
    try {
        ...
        const jumlahKeluar = parseInt(jumlah) || 0;
        const stokTersedia = barang.stok;
        const stokSetelahKeluar = stokTersedia - jumlahKeluar;

        let status = "aman";
        let pesan = "Stok mencukupi untuk transaksi";

        if (jumlahKeluar > stokTersedia) {
            status = "tidak_mencukupi";
            pesan = `Stok tidak mencukupi. Stok tersedia: ${stokTersedia}`;
        } else if (stokSetelahKeluar < barang.batas_minimal) {
            status = "peringatan";
            pesan = `Transaksi akan membuat stok di bawah batas minimal (${barang.batas_minimal})`;
        }
        ...
        res.status(200).json({
            ...
            data: {
                ...
                stok_setelah_keluar: Math.max(0, stokSetelahKeluar),
        ...
        res.status(500).json({
            success: false,
            message: "Gagal melakukan pengecekan stok",
            error: error.message
        });
    }
};